...
def _cache_dir() -> str | None:
    """Pick a writable directory for numba's on-disk cache, or None to compile every start.

    A cache only helps when the same filesystem serves several games; when it does, init
    drops from a full compile to a load. Nothing here is required for correctness."""
    if os.environ.get("SAGE_NUMBA_CACHE", "1") != "1":
        return None
    here = os.path.dirname(os.path.abspath(__file__))
    candidates = [os.path.join(here, "__numba_cache__"), tempfile.gettempdir(), None]
    if os.environ.get("NUMBA_CACHE_DIR"):
        candidates.insert(0, os.environ["NUMBA_CACHE_DIR"])
    for cand in candidates:
        try:
            if cand is None:
                cand = tempfile.mkdtemp(prefix="sage_numba_")
            elif cand == tempfile.gettempdir():
                cand = os.path.join(cand, "sage_numba_cache")
            os.makedirs(cand, exist_ok=True)
            with tempfile.TemporaryFile(dir=cand):
                pass
            return cand
        except Exception:
            continue
...
NUMBA_CACHE = _cache_dir()
if NUMBA_CACHE:
    os.environ["NUMBA_CACHE_DIR"] = NUMBA_CACHE
import numba  # noqa: E402  (after the cache directory is chosen)
try:
    numba.config.reload_config()
except Exception:
    pass
from numba import boolean, int8, int16, int32, int64, float64, njit  # noqa: E402


def fixed(sig, **kw):
    """njit with one pinned signature. Compiles eagerly, exactly once, and then refuses new
    specialisations, so literal or differently sized integer arguments are cast instead of
    triggering a second compile. This keeps import inside the 30 s init budget.

    The on-disk cache is best effort: any failure to read or write it falls back to a plain
    in-memory compile of the same function."""
    def deco(fn):
        d = None
        if NUMBA_CACHE:
            try:
                d = njit(sig, cache=True, nogil=True, error_model="numpy", **kw)(fn)
            except Exception:
                d = None
        if d is None:
            d = njit(sig, cache=False, nogil=True, error_model="numpy", **kw)(fn)
        d.disable_compile()
        return d
    return deco
...
def new_state() -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    hs = np.zeros(HIST, dtype=np.int64)
...
def compute_hash(bd, st):
    ...
    ep = st[ST_EP]
    if ep != 0:
        # only hash ep if a capture is actually possible
        if st[ST_SIDE] == WHITE:
            if bd[ep - 11] == WP or bd[ep - 9] == WP:
                h ^= Z_EP[ep]
        else:
            if bd[ep + 11] == BP or bd[ep + 9] == BP:
                h ^= Z_EP[ep]
    if st[ST_SIDE] == BLACK:
        h ^= Z_SIDE
    return h
...
# --- move generation ------------------------------------------------------------------------------
@fixed((int8[::1], int32[::1], int32[::1], boolean))
...
# --- make / unmake ---------------------------------------------------------------------------------
@fixed((int8[::1], int32[::1], int32[:, ::1], int32))
...
@fixed((int8[::1], int32[::1], int32[:, ::1], int64[::1], int32))
def make_move(bd, st, undo, hs, m):
    ...
    if m & F_DOUBLE:
        ...
        nep = st[ST_EP]
        if side == WHITE:
            if bd[nep + 11] == BP or bd[nep + 9] == BP:
                h ^= Z_EP[nep]
        else:
            if bd[nep - 11] == WP or bd[nep - 9] == WP:
                h ^= Z_EP[nep]
    else:
        st[ST_EP] = 0
...
@fixed((int8[::1], int32[::1], int32[:, ::1], int64[::1]))
