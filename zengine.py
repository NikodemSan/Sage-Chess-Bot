...
class Engine:
    ...
    def search(self, fen: str, soft_ms: float, hard_ms: float, max_depth: int = 64, verbose: bool = False,
        ...
        while depth <= max_depth and not self.abort:
            ...
            # Redistribute rather than spend more: a best move that just changed or a score that
            # fell sharply earns another iteration; a move stable for several iterations stops early.
            dropped = depth >= 6 and iter_prev - int(score) >= 50
            if dropped:
                factor = 1.2
            elif changed:
                factor = 1.0
            else:
                factor = 0.5 if stable < 3 else 0.4
            limit = soft_ms * factor
            if elapsed > limit:
                break
            depth += 1
        self.st[zb.ST_PLY] = 0
