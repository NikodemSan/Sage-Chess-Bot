...
from zboard import (fixed,SQ120,SQ64,EMPTY,ST_SIDE,ST_HALF,ST_KSQ_W,ST_KSQ_B,WHITE,BLACK,
...
MOPUP_ENABLED = os.environ.get('SAGE_STAGE3E_MOPUP','1') == '1'
MOPUP_MAX_PIECES = 6
...
@fixed((int64, int64, int64, int64))
def feature(p,sq,side,bucket):
    if side==1:
        p=((p-1+6)%12)+1; sq^=56
    return bucket*768+(p-1)*64+sq

@fixed((int8[::1], int32[:, :, ::1], int64, int64, int16[:, ::1]))
...
@fixed((int8[::1], int32[:, :, ::1], int64, int16[:, ::1], int16[::1]))
...
@fixed((int32[:, :, ::1], int64, int64, int64, int64, int64))
...
@fixed((int8[::1], int32[::1], int32, int32[:, :, ::1], int64, int16[:, ::1]))
...
def mop_up_term(bd,st,acc,ply,h,count):
    ...
    metadata slots, so this hot-path check does not scan the board. The
    board is only read for bishop+knight, to find the bishop's colour.
    """
    ...
    winner=-1
    kbn=False
    if bnon==0 and ((wmat>=MOPUP_MIN_MATERIAL and wmajor>0) or (wnon==2 and wmat==660)):
        winner=WHITE
    elif wnon==0 and ((bmat>=MOPUP_MIN_MATERIAL and bmajor>0) or (bnon==2 and bmat==660)):
        winner=BLACK
    elif bnon==0 and wnon==2 and wmat==650:
        winner=WHITE; kbn=True
    elif wnon==0 and bnon==2 and bmat==650:
        winner=BLACK; kbn=True
    else:
        return 0
    if winner==WHITE:
    ...
    if kbn:
        # Bishop and knight: mate only happens in a corner of the bishop's colour,
        # so reward distance to the nearer such corner (a1/h8 dark, a8/h1 light).
        bpiece=3 if winner==WHITE else 9
        bc=0
        for i in range(64):
            if bd[SQ120[i]]==bpiece:
                bc=((i>>3)+(i&7))&1
                break
        lf=lksq&7; lr=lksq>>3
        if bc==0:
            d=min(max(lf,lr),max(7-lf,7-lr))
        else:
            d=min(max(lf,7-lr),max(7-lf,lr))
        bonus+=24*(7-d)
    return bonus if st[ST_SIDE]==winner else -bonus

@fixed((int8[::1], int32[::1], int64, int32[:, ::1], int32[:, ::1], int32[:, :, ::1], int64, int16[:, ::1], int16[::1], int32))
def evaluate(bd,st,mode,tbl_mg,tbl_eg,acc,ply,w1,w2,b2):
    ...
    # Scale with material left on the board (trade when ahead, keep pieces when behind) and
    # fade toward zero as the fifty-move counter grows so a winning side keeps making progress.
    wmat=int(acc[ply,1,h+12])&0xFFFF; bmat=int(acc[ply,1,h+13])&0xFFFF
    result=result*(208+(wmat+bmat)//120)//256
    result-=result*int(st[ST_HALF])//212
    if MOPUP_ENABLED and count<=MOPUP_MAX_PIECES:
        result+=mop_up_term(bd,st,acc,ply,h,count)
