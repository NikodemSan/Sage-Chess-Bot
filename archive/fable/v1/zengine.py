...
class Engine:
    ...
    def search(self, fen: str, soft_ms: float, hard_ms: float, max_depth: int = 64, verbose: bool = False) -> tuple[str, int, int]:
        ...
        self.sinfo[zs.SI_GEN] = (self.sinfo[zs.SI_GEN] + 1) & 0xFF
        self.killers[:] = 0
        self.history //= 2
    ...
    # --- pondering ----------------------------------------------------------------------------------
    def start_ponder(self, fen: str) -> None:
        """Search fen (opponent to move) in the background until stop_ponder is called."""
        self.stop_ponder()
        self.pondering = True
        self.abort = False
        ...
        def run() -> None:
            try:
                if not self.abort:
                    self.search(fen, 1e9, 1e9, max_depth=40)
            except Exception:  # noqa: BLE001 - pondering must never take the process down
                pass
            finally:
                self.pondering = False

        self._ponder = threading.Thread(target=run, daemon=True)
        self._ponder.start()
    ...
    def stop_ponder(self) -> None:
        ...
        if t is not None:
            self.abort = True
            # keep re-asserting the stop until the thread is really gone: it may be anywhere
            # between starting and resetting its own counters
            while t.is_alive():
                self.tinfo[0] = 0.0
                self.sinfo[zs.SI_STOP] = 1
