...
def is_repetition(hs, hl, ply, half):
    """Twofold repetition of the current position within the last `half` plies."""
    cur = hs[hl + ply]
    i = ply - 2
    lim = ply - half
    if lim < -hl:
...
@njit(int64(int8[::1], int32[::1], int32[:, ::1], int64[::1], int32[:, ::1], int32[:, ::1], int64, int64, int64[::1], float64[::1], int64, int32[:, ::1], int32[:, ::1], int16[:, :, ::1], int16[:, ::1], int16[::1], int32, int64), cache=False, nogil=True, error_model="numpy")
def qsearch(bd, st, undo, hs, mvbuf, scbuf, alpha, beta, sinfo, tinfo, mode, tbl_mg, tbl_eg, acc, w1, w2, b2, qply):
    ...
    if incheck and qply < 2:
        stand = -INF
        n = gen_moves(bd, st, mvbuf[ply], False)
    else:
        stand = evaluate(bd, st, mode, tbl_mg, tbl_eg, acc, ply, w2, b2)
        if stand >= beta:
            return stand
        if stand > alpha:
            alpha = stand
    ...
    for i in range(n):
        ...
        if stand > -INF:
            ...
            if m & F_CAPTURE:
                ...
                fr = m & M_FROM
                if PIECE_VAL[bd[fr]] > PIECE_VAL[vic] + 50 and see_capture(bd, st, m) < -50 and not (m & M_PROMO):
                    continue
        if mode >= 1:
...
@njit(int64(int8[::1], int32[::1], int32[:, ::1], int64[::1], int32[:, ::1], int32[:, ::1], int64, int64, int64, int64[::1], float64[::1], int64[::1], int64[::1], int32[:, ::1], int32[:, ::1], int64, int32[:, ::1], int32[:, ::1], int16[:, :, ::1], int16[:, ::1], int16[::1], int32, boolean), cache=False, nogil=True, error_model="numpy")
def negamax(bd, st, undo, hs, mvbuf, scbuf, depth, alpha, beta, sinfo, tinfo, tt_key, tt_data, killers, history,
    ...
    static_eval = 0
    if not incheck:
        static_eval = evaluate(bd, st, mode, tbl_mg, tbl_eg, acc, ply, w2, b2)
        if tt_hit:
    ...
    if not pv_node and not incheck:
        ...
        if depth <= 7 and static_eval - 75 * depth >= beta and static_eval < MATE_BOUND:
            return static_eval
        # null move pruning
        if can_null and depth >= 3 and static_eval >= beta and has_non_pawn(bd, side):
            r = 3 + depth // 3
