...
# Capture history is intentionally small and conservative. It only refines ordering *inside*
# the existing cheap-SEE good/bad capture buckets; it never changes pruning or qsearch.  Keeping
# MVV-LVA dominant across large victim-value gaps prevents a few noisy cutoffs from turning this
# into an over-trained tactical policy.
CAP_HIST_MAX = 4096
CAP_HIST_MVV_SCALE = 64


@njit(cache=False, nogil=True, error_model="numpy")
def update_capture_history(capture_history, pc, to, vic_type, bonus):
    """Bounded history-gravity update without a table-wide rescale.

    This is the cheap part that makes capture history suitable for the one-core tournament: one
    indexed load/store on a beta cutoff, rather than SEE-like attack reconstruction at every node.
    """
    if bonus > 512:
        bonus = 512
    elif bonus < -512:
        bonus = -512
    old = int(capture_history[pc, to, vic_type])
    # History gravity: large existing values resist further reinforcement and decay naturally
    # under contradictory evidence.  Integer-only arithmetic is friendly to Numba's hot path.
    val = old + bonus - (old * abs(bonus)) // CAP_HIST_MAX
    if val > CAP_HIST_MAX:
        val = CAP_HIST_MAX
    elif val < -CAP_HIST_MAX:
        val = -CAP_HIST_MAX
    capture_history[pc, to, vic_type] = val
...
@njit(cache=False, nogil=True, error_model="numpy")
def cheap_see_capture(bd, st, m):
    """Fast defended-target approximation used below the SEE gate and in qsearch."""
    fr = m & M_FROM
    to = (m & M_TO) >> 7
    att = bd[fr]
    vic = bd[to]
...
@njit(cache=False, nogil=True, error_model="numpy")
def score_moves(bd, st, mv, sc, n, tt_move, killers, history, capture_history, ply, depth):
    for i in range(n):
        m = mv[i]
        if m == tt_move:
            sc[i] = 20000000
            ...
            if vic == EMPTY:
                vic = WP  # en passant
            att = bd[fr]
            # Learned cutoff success refines MVV-LVA but cannot cross the existing SEE bucket.
            # Victim value remains the dominant signal; history mainly learns which attacker/to
            # combinations work best among otherwise comparable captures.
            s = MVV_LVA[vic, att] * CAP_HIST_MVV_SCALE + capture_history[att, to, PIECE_TYPE[vic]] // 128
            ...
            if PIECE_VAL[att] > PIECE_VAL[vic]:
                # Exact SEE is only capable of *rescuing* a capture that v5's cheap test would
                # demote. If the destination is not defended, the cheap result is already >= 0
                # and a full swap-off cannot improve the ordering decision.
                cheap_losing = cheap_see_capture(bd, st, m) < 0
                losing = cheap_losing
                if depth >= 10 and cheap_losing:
                    losing = not see_ge(bd, st, m, 0)
                sc[i] = (1000000 + s) if losing else (10000000 + s)
            else:
                sc[i] = 10000000 + s
            if m & M_PROMO:
...
def qsearch(bd, st, undo, hs, mvbuf, scbuf, alpha, beta, sinfo, tinfo, eval_key, eval_val, mode, tbl_mg, tbl_eg, acc, w1, w2, b2, qply):
    ...
    # Stand-pat is never legal while in check.  Unlimited recursive check-evasion qsearch,
    # however, was measured to inflate the tree dramatically at tournament time controls.
    # Expand checks fully for two qsearch plies, then switch to a bounded evasion frontier:
    # every legal evasion is still considered, but quiet evasions are evaluated as leaves while
    # captures/promotions may continue through normal qsearch.  This preserves legality/mate
    # handling without paying for arbitrarily long quiet checking sequences.
    bounded_evasion = incheck and qply >= 2
    if incheck:
        stand = -INF
    ...
    for i in range(n):
        ...
        if stand > -INF and not sparse:
            ...
            if m & F_CAPTURE:
                ...
                fr = m & M_FROM
                if PIECE_VAL[bd[fr]] > PIECE_VAL[vic] + 50 and cheap_see_capture(bd, st, m) < -50 and not (m & M_PROMO):
                    continue
        if mode >= 1:
        ...
        if not make_move(bd, st, undo, hs, m):
            continue
        legal += 1
        if bounded_evasion and (m & (F_CAPTURE | M_PROMO)) == 0:
            # We cannot stand-pat in the checked parent, but a quiet legal evasion need not open
            # another unbounded qsearch subtree.  Score the resulting legal position directly.
            # Terminal/draw handling is retained so mates and claimable draws are never hidden.
            child_ply = st[ST_PLY]
            child_side = st[ST_SIDE]
            child_hl = st[ST_HISTLEN]
            child_ksq = st[ST_KSQ_W] if child_side == WHITE else st[ST_KSQ_B]
            child_incheck = attacked(bd, child_ksq, child_side ^ 1)
            child_draw = child_ply > 0 and (st[ST_HALF] >= 100 or is_repetition(hs, child_hl, child_ply, st[ST_HALF]))
            if child_draw:
                child_score = -sinfo[SI_CONTEMPT] if child_side == sinfo[SI_ROOT_SIDE] else sinfo[SI_CONTEMPT]
            elif child_incheck and not has_legal_move(bd, st, undo, hs, mvbuf[child_ply]):
                child_score = -MATE + child_ply
            else:
                child_score = cached_eval(hs[child_hl + child_ply], bd, st, mode, tbl_mg, tbl_eg, acc, child_ply, w2, b2, eval_key, eval_val)
            score = -child_score
        else:
...
@njit(int64(int8[::1], int32[::1], int32[:, ::1], int64[::1], int32[:, ::1], int32[:, ::1], int64, int64, int64, int64[::1], float64[::1], int64[::1], int32[::1], int64[::1], int64[::1], int32[:, ::1], int32[:, ::1], int16[:, :, ::1], int64, int32[:, ::1], int32[:, ::1], int16[:, :, ::1], int16[:, ::1], int16[::1], int32, boolean), cache=False, nogil=True, error_model="numpy")
def negamax(bd, st, undo, hs, mvbuf, scbuf, depth, alpha, beta, sinfo, tinfo, eval_key, eval_val, tt_key, tt_data, killers, history, capture_history,
    ...
    if not pv_node and not incheck:
        ...
        if can_null and depth >= 3 and static_eval >= beta and null_move_material_ok(bd, side):
            ...
            score = -negamax(bd, st, undo, hs, mvbuf, scbuf, depth - 1 - r, -beta, -beta + 1, sinfo, tinfo, eval_key, eval_val, tt_key, tt_data,
                             killers, history, capture_history, mode, tbl_mg, tbl_eg, acc, w1, w2, b2, False)
            unmake_null(st, undo)
            if sinfo[SI_STOP]:
                return 0
            if score >= beta:
    ...
    sc = scbuf[ply]
    n = gen_moves(bd, st, mv, False)
    score_moves(bd, st, mv, sc, n, tt_move, killers, history, capture_history, ply, depth)
    ...
    for i in range(n):
        ...
        if legal == 1:
            score = -negamax(bd, st, undo, hs, mvbuf, scbuf, depth - 1, -beta, -alpha, sinfo, tinfo, eval_key, eval_val, tt_key, tt_data,
                             killers, history, capture_history, mode, tbl_mg, tbl_eg, acc, w1, w2, b2, True)
        else:
            red = 0
        ...
        if score > best:
            ...
            if score > alpha:
                ...
                if alpha >= beta:
                    if m & F_CAPTURE:
                        # Learn only from main-search beta cutoffs.  Do not train in qsearch and do
                        # not use history to prune: for Fable it is a cheap ordering hint only.
                        fr = m & M_FROM
                        to = (m & M_TO) >> 7
                        pc = bd[fr]
                        vic = bd[to]
                        if vic == EMPTY:
                            vic = BP if side == WHITE else WP  # en passant victim colour
                        bonus = depth * depth * 8
                        if bonus > 512:
                            bonus = 512
                        update_capture_history(capture_history, pc, to, PIECE_TYPE[vic], bonus)
                        # Captures searched before the cutoff are negative evidence.  Penalise less
                        # strongly than the winner to avoid thrashing on tactical move-order noise.
                        malus = -(bonus // 2)
                        for j in range(i):
                            pm = mv[j]
                            if pm & F_CAPTURE:
                                ...
                                pvic = bd[pto]
                                if pvic == EMPTY:
                                    pvic = BP if side == WHITE else WP
                                update_capture_history(capture_history, ppc, pto, PIECE_TYPE[pvic], malus)
                    elif is_quiet:
                        if killers[ply, 0] != m:
