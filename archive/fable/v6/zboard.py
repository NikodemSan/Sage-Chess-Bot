...
# --- hashing -------------------------------------------------------------------------------------
@njit(cache=False, nogil=True, error_model="numpy")
def ep_hashable(bd, st):
    """Return True iff the side to move has at least one *legal* en-passant capture.

    FIDE repetition identity distinguishes an en-passant target only when it changes the legal
    move set.  Merely having an adjacent pawn is not enough: that pawn can be pinned.  Zobrist
    hashing therefore performs the tiny EP capture simulation here before including Z_EP.
    """
    ep = st[ST_EP]
    if ep == 0:
        return False
    ...
    if side == WHITE:
        ...
        s2 = ep + 9
        ksq = st[ST_KSQ_B]
    if bd[cap_sq] != captured or bd[ep] != EMPTY:
        return False
    for fr in (s1, s2):
        if bd[fr] != pawn:
            continue
        ...
        bd[cap_sq] = EMPTY
        bd[ep] = pawn
        legal = not attacked(bd, ksq, side ^ 1)
        bd[ep] = EMPTY
        bd[cap_sq] = captured
        bd[fr] = pawn
        if legal:
...
def make_move(bd, st, undo, hs, m):
    ...
    if m & F_DOUBLE:
        st[ST_EP] = (fr + to) >> 1
        # The new EP right belongs to the opponent, so test legality from that side before
        # folding the target into the incremental key.
        st[ST_SIDE] = side ^ 1
        if ep_hashable(bd, st):
            h ^= Z_EP[st[ST_EP]]
        st[ST_SIDE] = side
    else:
        st[ST_EP] = 0
