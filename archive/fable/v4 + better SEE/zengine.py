    ...
    def search(
        ...
        *,
        ...
        while depth <= max_depth:
            ...
            if depth >= 5:
                limit = soft_ms * (1.05 if (changed or swing) else 0.48)
                if elapsed >= limit:
                    break
            depth += 1
...
def budget(
    ...
    *our* 120-second clock has already been spent.  Budget from moves made by this process instead.
    """
    tl = max(float(time_left_ms) - 180.0, 40.0)  # protocol + Python safety reserve
    ...
    moves_left = max(18.0, 38.0 - 0.45 * float(moves_made))
    soft = tl / moves_left + float(increment_ms) * 0.72
    ...
    soft = min(soft, tl * 0.22)
    hard = min(tl * 0.36, max(soft * 3.5, soft + 30.0))
