...
def clip_hidden(x):
    ...
    return x
@njit(cache=False)
def evaluate_nnue_scratch(acc2,side,board,x,h):
    """Bit-exact HalfKP inference using caller-owned clip/hidden buffers."""
    first=0 if side==WHITE else 1;second=1-first
    for j in range(NNUE_DIM):
        x[j]=np.int16(clip_ft(int(acc2[first,j])))
        x[NNUE_DIM+j]=np.int16(clip_ft(int(acc2[second,j])))
    for u in range(32):
        total=np.int64(H1_B[u])
        for j in range(2*NNUE_DIM):total+=np.int64(H1_W[u,j])*np.int64(x[j])
        h[u]=clip_hidden(int(total//H1_DIV))
    for u in range(32):
        total=np.int64(H2_B[u])
        for j in range(32):total+=np.int64(H2_W[u,j])*h[j]
        h[32+u]=clip_hidden(int(total//H2_DIV))
    bucket=0
    if USE_BUCKETS:
        cnt=0
        for s in range(64):
    ...
    for j in range(32):total+=np.int64(OUT_W[bucket,j])*h[32+j]
    return int((total*CP_SCALE)//(HEAD_SCALE*FT_SCALE))

@njit(cache=False)
def evaluate_nnue(acc2,side,board):
    # External/root calls are infrequent, so a local scratch allocation is fine.
    x=np.empty(2*NNUE_DIM,np.int16);h=np.empty(64,np.int64)
    return evaluate_nnue_scratch(acc2,side,board,x,h)

@njit(cache=False)
def rebuild_one_ks(board,persp,out,king):
    for j in range(NNUE_DIM):out[j]=int(FT_B[j])
    if king<0:return
    for sq in range(64):
        pc=int(board[sq])
...
@njit(cache=False)
def rebuild_both_ks(board,out2,wk,bk):
    rebuild_one_ks(board,WHITE,out2[0],wk);rebuild_one_ks(board,BLACK,out2[1],bk)

@njit(cache=False)
def update_child_after_move_ks(board_after,m,side,captured,parent_acc,child_acc,wk,bk):
    for p in range(2):
        for j in range(NNUE_DIM):child_acc[p,j]=parent_acc[p,j]
    ...
    for pi in range(2):
        persp=WHITE if pi==0 else BLACK;king=wk if persp==WHITE else bk
        if is_king and persp==side:
            rebuild_one_ks(board_after,persp,child_acc[pi],king);continue
        if not is_king:
            add_feature(child_acc[pi],feature_index(king,fr,moving_before,persp),-1)
            add_feature(child_acc[pi],feature_index(king,to,moved_after,persp),1)
        if captured!=0:
            cs=to-8*side if fl==FLAG_EP else to
            add_feature(child_acc[pi],feature_index(king,cs,captured,persp),-1)
        if fl==FLAG_CASTLE:
            if to==6:rf,rt=7,5
            elif to==2:rf,rt=0,3
            elif to==62:rf,rt=63,61
            ...
            rook=side*R
            add_feature(child_acc[pi],feature_index(king,rf,rook,persp),-1)
            add_feature(child_acc[pi],feature_index(king,rt,rook,persp),1)
