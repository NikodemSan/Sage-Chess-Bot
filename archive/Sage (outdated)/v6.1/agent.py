"""Sage Stage 8E. Exact Stage 7D search plus narrowly gated bare-king mop-up evaluation."""
import os
...
BUILD='sage-stage8e-bareking-mopup-20260910'
...
FEATURES.update(stage7_lean='D',improving_lmr=False,history_lmr=False,continuation=zsearch.CONTINUATION,
                ...
                mopup_king_weight=zeval.MOPUP_KING_WEIGHT)
...
def get_move(fen:str,time_left_ms:int)->str:
    ...
    print(f"S8E {move} d{depth} s{score} n{int(ENGINE.sinfo[0])} t{ENGINE.last_info['elapsed_ms']} stop{int(ENGINE.sinfo[1])}",file=sys.stderr,flush=True)
    return move
