...
class Engine:
    def __init__(self, nnue_path: str | None = None, tt_bits: int = zs.TT_BITS) -> None:
        ...
        self.stack = np.zeros(zb.MAX_PLY + 2, dtype=np.int32)
        self.continuation = np.zeros((768, 768) if zs.CONTINUATION else (1,1), dtype=np.int32)
        size = 1 << tt_bits
        assert size == zs.TT_SIZE, "TT size is compiled into the search"
        self.tt_key = np.zeros(size*(2 if zs.TT_TWO_CONTEXTS and not zs.TT_COMPACT else 1), dtype=np.int64)
        self.tt_data = np.zeros(size*(2 if zs.TT_TWO_CONTEXTS else 1), dtype=np.int64)
        self.tt_context = np.zeros(size*(2 if zs.TT_TWO_CONTEXTS else 1), dtype=np.int64)
        self.sinfo = np.zeros(21, dtype=np.int64)
        self.contempt = 0
    ...
    def set_position(self, fen: str) -> None:
        ...
        for i, x in enumerate(self.game_hashes):
            self.hs[i] = x
        # Rebuild reversible-history fingerprint at each real root.
        total=sum(self.game_hashes[max(0,hl-int(self.st[zb.ST_HALF])):hl])
        self.hs[zb.HIST+hl]=((total+(1<<63))%(1<<64))-(1<<63)
        if self.mode >= 1:
    ...
    def search(self, fen: str, soft_ms: float, hard_ms: float, max_depth: int = 64, verbose: bool = False) -> tuple[str, int, int]:
        ...
        self.history //= 2
        self.stack[:] = 0
        if zs.CONTINUATION:self.continuation //= 2
        ...
        while depth <= max_depth and not self.abort:
            window = int(os.environ.get("SAGE_STAGE4_ASPIRATION","50")) if depth >= 5 else zs.INF
            ...
            while True:
                ...
                score = zs.negamax(self.bd, self.st, self.undo, self.hs, self.mvbuf, self.scbuf, depth, alpha, beta,
                                   self.sinfo, self.tinfo, self.tt_key, self.tt_data, self.tt_context, self.killers, self.history, self.stack, self.continuation,
        ...
        self.last_info={'depth':last_depth,'score_cp':best_score,'forced_move':False,
                        'fallback':last_depth==0,'iterations':iterations,
                        'partial_iteration_used':bool(self.sinfo[zs.SI_STOP] and rb!=0 and rb in legal),
                        'stats':{'nodes':int(self.sinfo[0]),'stop':int(self.sinfo[1]),
                                 'seldepth':int(self.sinfo[zs.SI_SELDEPTH])}}
        names=('qnodes','draw_claims','tt_cutoffs','tt_context_misses','null_attempts','null_cutoffs','reductions','researches','quiet_prunes','q_capture_prunes')
        self.last_info['stats'].update({name:int(self.sinfo[10+j]) for j,name in enumerate(names)})
        self.last_info['stats']['tt_secondary_hits']=int(self.sinfo[20])
        self.last_info['aspiration_retries']=aspiration_retries
        self.last_info['pv_source']='transposition_table_not_certified_search_line'
