"""Sage Stage 8F. Stage 8E plus root-only fifty-move zeroing-move ordering."""
import os
...
BUILD='sage-stage8f-root-zeroing-order-20260910'
...
FEATURES.update(stage7_lean='D',improving_lmr=False,history_lmr=False,continuation=zsearch.CONTINUATION,
                endgame_mopup=zeval.MOPUP_ENABLED,mopup_max_pieces=zeval.MOPUP_MAX_PIECES,
                mopup_min_material_cp=zeval.MOPUP_MIN_MATERIAL,mopup_edge_weight=zeval.MOPUP_EDGE_WEIGHT,
                mopup_king_weight=zeval.MOPUP_KING_WEIGHT,conversion_progress_ordering=zsearch.PROGRESS_ORDERING,
                progress_scope='root_only',progress_max_pieces=zsearch.PROGRESS_MAX_PIECES,
                progress_min_material_edge=zsearch.PROGRESS_MIN_MATERIAL_EDGE,progress_min_static_edge=zsearch.PROGRESS_MIN_STATIC_EDGE,
                progress_half_start=zsearch.PROGRESS_HALF_START,progress_half_full=zsearch.PROGRESS_HALF_FULL,
                progress_max_quiet_bonus=zsearch.PROGRESS_MAX_QUIET_BONUS)
...
def get_move(fen:str,time_left_ms:int)->str:
    ...
    print(f"S8F {move} d{depth} s{score} n{int(ENGINE.sinfo[0])} t{ENGINE.last_info['elapsed_ms']} stop{int(ENGINE.sinfo[1])}",file=sys.stderr,flush=True)
    return move
