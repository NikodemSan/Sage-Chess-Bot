...
def ep_hash(bd, st, side, ep):
    """FIDE position identity includes en passant only if a LEGAL capture exists."""
    if ep == 0:
        return np.int64(0)
    pawn = WP if side == WHITE else BP
    step = 10 if side == WHITE else -10
    captured_sq = ep - step
    victim = bd[captured_sq]
    ...
    for delta in (-1, 1):
        ...
        if bd[fr] != pawn:
            continue
        bd[fr] = EMPTY
        bd[captured_sq] = EMPTY
        bd[ep] = pawn
        legal = not attacked(bd, king, side ^ 1)
        bd[fr] = pawn
        bd[captured_sq] = victim
        bd[ep] = EMPTY
        if legal:
            return Z_EP[ep]
    return np.int64(0)
...
def compute_hash(bd, st):
    ...
    h ^= ep_hash(bd, st, st[ST_SIDE], st[ST_EP])
    if st[ST_SIDE] == BLACK:
        h ^= Z_SIDE
    return h
...
def make_move(bd, st, undo, hs, m):
    ...
    if m & F_DOUBLE:
        st[ST_EP] = (fr + to) >> 1
        h ^= ep_hash(bd, st, side ^ 1, st[ST_EP])
    else:
        st[ST_EP] = 0
