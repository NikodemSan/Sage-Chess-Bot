...
from zboard import (
    fixed,    BK, BP, EMPTY, F_CAPTURE, F_EP, M_FROM, M_PROMO, M_TO, PIECE_TYPE, PIECE_VAL, SQ120, ST_HALF,
...
INF = 32000
DRAW = 0
# The staged-ordering experiment (searching the hash move before generating the rest)
# regressed and its code has been removed from the search body to keep compile time down.
STAGED_TT = False
...
@fixed((int64, int64, int64, int64, int64))
...
@fixed((int64[::1], int64[::1], int64, int64, int64, int64, int64, int64, int64))
...
@fixed((int64[::1], int64, int64, int64))
def is_repetition(hs, hl, ply, half):
    """Draw when the position occurred once before strictly inside the search tree, or twice
    before counting the game history (a true threefold). Null branches are approximate."""
    matches = 0
    cur = hs[hl + ply]
    i = ply - 2
    lim = ply - half
...
@fixed((int8[::1], int32[::1], int32[::1], int32[::1], int64, int64, int32[:, ::1], int32[:, ::1], int64))
...
@fixed((int32[::1], int32[::1], int64, int64))
...
@fixed((int8[::1], int32[::1], int32[:, ::1], int64[::1], int32[::1]))
...
@fixed((int8[::1], int32[:, :, ::1], int64, int16[:, ::1]))
...
@fixed((int8[::1], int32[::1], int32[:, ::1], int64[::1], int64))
...
@fixed(int64(int8[::1], int32[::1], int32[:, ::1], int64[::1], int32[:, ::1], int32[:, ::1], int64, int64, int64[::1], float64[::1], int64, int32[:, ::1], int32[:, ::1], int32[:, :, ::1], int16[:, ::1], int16[::1], int32, int64))
...
@fixed(int64(int8[::1], int32[::1], int32[:, ::1], int64[::1], int32[:, ::1], int32[:, ::1], int64, int64, int64, int64[::1], float64[::1], int64[::1], int64[::1], int32[:, ::1], int32[:, ::1], int64, int32[:, ::1], int32[:, ::1], int32[:, :, ::1], int16[:, ::1], int16[::1], int32, boolean))
def negamax(bd, st, undo, hs, mvbuf, scbuf, depth, alpha, beta, sinfo, tinfo, tt_key, tt_data, killers, history,
            mode, tbl_mg, tbl_eg, acc, w1, w2, b2, can_null):  # noqa: E501
    ply = st[ST_PLY]
    if CORRECTIONS and common_dead_material(bd,acc,ply,w1): return 0
    hl = st[ST_HISTLEN]
    is_root = ply == 0
    ...
    if incheck:
        depth += 1
    # Runtime booleans (not literals): numba would otherwise specialise the recursive call
    # on Literal[bool](True/False) and compile this function three times at import.
    null_ok = ply >= 0
    null_off = ply < 0
    ...
    if not pv_node and not incheck:
        ...
        if depth <= 8 and static_eval - 70 * depth >= beta and static_eval < MATE_BOUND:
        ...
        if can_null and depth >= 3 and static_eval >= beta and has_non_pawn(bd, side) and has_legal_move(bd, st, undo, hs, mvbuf[ply]):
            r = 3 + depth // 3 + min(2, (static_eval - beta) // 150)
            ...
            score = -negamax(bd, st, undo, hs, mvbuf, scbuf, depth - 1 - r, -beta, -beta + 1, sinfo, tinfo, tt_key, tt_data,
                             killers, history, mode, tbl_mg, tbl_eg, acc, w1, w2, b2, null_off)
            unmake_null(st, undo)
            if sinfo[SI_STOP]:
                return 0
            if score >= beta:
    ...
    alpha_orig = alpha
    legal = 0
    futile = (not pv_node) and (not incheck) and depth <= 4 and static_eval + 100 + 120 * depth <= alpha
    ...
    for i in range(n):
        ...
        if legal > 0 and is_quiet and not pv_node and not incheck:
            if depth <= 5 and legal >= lmp_limit:
                continue
            if futile:
                continue
        if mode >= 1:
        ...
        if legal == 1:
            score = -negamax(bd, st, undo, hs, mvbuf, scbuf, depth - 1, -beta, -alpha, sinfo, tinfo, tt_key, tt_data,
                             killers, history, mode, tbl_mg, tbl_eg, acc, w1, w2, b2, null_ok)
        else:
            red = 0
